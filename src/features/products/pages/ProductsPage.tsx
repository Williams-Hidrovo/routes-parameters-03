...
const productosData: IProducto[] = [
  ...
  },
  {
    ...
    nombre: "Tablet 10''",
    precio: 300,
    descripcion: "Tablet ligera con 128GB de almacenamiento y stylus incluido.",
...
const ProductsPage = () => {
  ...
  const showLista =
    filtroActivo == ICategorias.Todos || filtroActivo == null
      ? productosData
      : listaFiltrada;
    ...
    <>
      ...
      {showLista.length > 0 ? (
        <section className="container">
          {showLista.map((product) => (
            <ProductCard key={product.id} product={product} />
          ))}
        </section>
      ) : (
        <p>No tenemos por el momento estos productos</p>
      )}
    </>
  );
};
